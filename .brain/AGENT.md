# Agent Instructions — nix-vps (public)
...
- **BRAIN_CONTEXT**: vps
- **BRAIN_REPO**: nix-vps
- **Purpose**: Public host-ownership layer for VPS infrastructure. Owns VPS NixOS flake outputs, base host modules, host profiles and disk layout, SOPS host bootstrap wiring, host selection and wiring for services from nix-services.
- **Private counterpart**: `../nix-vps-private/` (never pushed to GitHub)

---

## Public Brain Rules

- This .brain/ directory is pushed to Gitea and GitHub — never add sensitive content here
- This directory only contains content explicitly published via `brainctl publish`
- Raw investigations live in `../nix-vps-private/.brain/` — BRAIN_ROOT points there
...
Rule: if the change is reusable service behavior, it belongs in `nix-services`, not `nix-vps`.
...
3. `flake.nix`
4. relevant files under `nixos/modules/`, `nixos/profiles/`, `nixos/hosts/`, `nixos/disko/`
...
If a task touches shared module behavior, follow pointers to `nix-services` instead of duplicating logic here.
...
- Before build/rebuild, run `nix run "path:$PWD#validate-private-config" -- vps-01`
- For `nix build` and `nixos-rebuild`, pass `--override-input private "path:${NIX_VPS_PRIVATE_FLAKE:-$PWD/../nix-vps-private}"`
...
- Start each session with `git fetch origin && git pull --rebase origin main && git status --short --branch`
...
- Private continuity notes, real hostnames/IPs, and operator-specific workflow belong in `../nix-vps-private/records/`

---

## Build, Deploy, and Validation Norms

- Validate private wiring before evaluation/build: `nix run "path:$PWD#validate-private-config" -- vps-01`
...
Each service area can have an investigation file under `../nix-vps-private/records/<service>/INVESTIGATION.md`.
...
- Change reusable service module behavior → work in `nix-services`
- Enable or wire existing shared services on VPS host → work in `nix-vps` + `nix-vps-private`
- Add/rotate secrets → update SOPS-encrypted data in `nix-vps-private`, reference `/run/secrets/...`
- Change host-only hardening/network/user policy → work in `nix-vps/nixos/modules/`
